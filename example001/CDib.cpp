...
BOOL CDib24::ReadBMPFile(LPCWSTR lpszFilename)
{
    ...
    {
        return perrDialog->DispErrorMsg(L"ファイル読込", L"ファイルオープンに失敗");
    }
    ...
    {
        return perrDialog->DispErrorMsg(L"ファイル読込", L"BITMAPFILEHEADER構造体の読み込みに失敗");
    }

    // ファイルがBMPであるか判定
    if(bmfh.bfType != 0x4d42)
    {
        return perrDialog->DispErrorMsg(L"ファイル読込", L"BMP形式ではないファイルを指定");
    }

    // BITMAPINFO構造体のメモリ割り当て
    if(AllocBMInfoMem() == FALSE)
    {
        return perrDialog->DispErrorMsg(L"ファイル読込", L"BITMAPINFO構造体のメモリ割り当てに失敗");
    }
    ...
    {
        return perrDialog->DispErrorMsg(L"ファイル読込", L"BITMAPINFOHEADER構造体の読み込みに失敗");
    }

    // イメージが24bitカラーであるか判定
    if(lpbmi->bmiHeader.biBitCount != 24)
    {
        return perrDialog->DispErrorMsg(L"ファイル読込", L"24bitカラーではないBMPファイルを指定");
    }
    ...
    {
        return perrDialog->DispErrorMsg(L"ファイル読込", L"イメージビットのメモリ割り当てに失敗");
    }
    ...
    {
        return perrDialog->DispErrorMsg(L"ファイル読込", L"イメージビットの読み込みに失敗");
    }
...
BOOL CDib24::CreateDibObject(LONG lWidth, LONG lHeight)
{
    ...
    if(AllocBMInfoMem() == FALSE)
    {
        return perrDialog->DispErrorMsg(L"DIB生成", L"BITMAPINFO構造体のメモリ割り当てに失敗");
    }
    ...
    if(AllocBitsMem(uImageSize) == FALSE)
    {
        return perrDialog->DispErrorMsg(L"DIB生成", L"イメージビットのメモリ割り当てに失敗");
    }
...
////////////////////////////////////////////////////////////////
// 矩形の描画
void CDib24::DrawRectangle(RECT rcDst, COLORREF color, BYTE byAlpha)
{
    // スキャンラインと高さをセット
    int nCopyWidth, nCopyHeight;
    nCopyWidth = (rcDst.right - rcDst.left) * 3;
    ...
    // イメージビットのコピー
    int i, j;
    int nDstLine, nDstHeight, nDstIndex;
    LPBYTE lpDstBits;
    BYTE byDstColor, byCopyColor;
    ...
    for(i = 0; i < nCopyHeight; i++)
    {
        for(j = 0; j < nCopyWidth; j += 3)
        {
            ...
            byDstColor = lpDstBits[nDstIndex];
            byCopyColor = ((byBlue * byAlpha) + (byDstColor * (255 - byAlpha))) / 255;
            lpDstBits[nDstIndex] = byCopyColor;

            byDstColor = lpDstBits[nDstIndex + 1];
            byCopyColor = ((byGreen * byAlpha) + (byDstColor * (255 - byAlpha))) / 255;
            lpDstBits[nDstIndex + 1] = byCopyColor;

            byDstColor = lpDstBits[nDstIndex + 2];
            byCopyColor = ((byRed * byAlpha) + (byDstColor * (255 - byAlpha))) / 255;
            lpDstBits[nDstIndex + 2] = byCopyColor;
        }
    }
}
